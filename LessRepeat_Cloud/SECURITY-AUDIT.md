...
| F-003 | High | Stored XSS in the legacy voice console | `dashboard/public/console.html`, `dashboard/server.js` | CWE-79 | The legacy console inserted agent and provider fields through `innerHTML`. | The public legacy route now redirects to the text-node-based LessRepeat app. | Fixed and regression-tested |
...
No Critical or High findings remain in the changed LessRepeat paths.
...
- The unsafe legacy console is no longer reachable and redirects to the LessRepeat app.
