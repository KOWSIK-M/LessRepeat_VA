...
Owners create, list and revoke links from **Demo links** in Studio. Active links can be opened or copied during the browser session that created them. Expired, exhausted and revoked links fail closed. Public routes are `/demo/<token>`, `GET /api/public/demo/<token>` and `POST /api/public/demo/<token>/sGiession`.
