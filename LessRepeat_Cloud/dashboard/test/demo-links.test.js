...
test('encrypts a recoverable demo token without storing it in plaintext', () => {
  const generated = demoLinks.createDemoToken();
  const key = 'test-demo-link-encryption-key-32-characters';
  const ciphertext = demoLinks.sealDemoToken(generated.token, key);
  assert.match(ciphertext, /^v1\./);
  assert.equal(ciphertext.includes(generated.token), false);
  assert.equal(demoLinks.openDemoToken(ciphertext, key), generated.token);
  assert.equal(demoLinks.openDemoToken(ciphertext, key + '-wrong'), '');
  assert.equal(demoLinks.openDemoToken(ciphertext.slice(0, -2) + 'xx', key), '');
});
