...
async function speakWithSavedVoice(text) {
  stopCustomVoice();
  voiceRequest = new AbortController();
  try {
    const response = await fetch('/api/public/demo/' + encodeURIComponent(token) + '/tts', {
      method: 'POST', credentials: 'omit', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text }), signal: voiceRequest.signal
    });
    ...
    setStatus('speaking', 'Agent is speaking', 'You can interrupt naturally at any time.');
    await voiceAudio.play();
  } catch (error) {
    if (error.name !== 'AbortError') setStatus('error', 'Voice unavailable', 'End the call, then try once more.');
  } finally { voiceRequest = null; }
}

function initials(name) {
  return String(name || 'L').trim().split(/\s+/).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'L';
}
...
async function loadDemo() {
  ...
  try {
    ...
    if (metadata.agent.voice) byId('voiceLabel').textContent = 'Live voice agent · ' + metadata.agent.voice.label;
...
async function handleSignal(message) {
  ...
  if (message.type === 'rtf-bot-text') {
    ...
    if (text && voiceTransport === 'browser') speakWithSavedVoice(text);
    return;
  }
