...
async function waitForServer(baseUrl, child, readLogs) {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode !== null) {
      throw new Error(`LessRepeat server exited during startup.\n${readLogs()}`);
    }
    try {
  ...
  throw new Error(`LessRepeat server did not become ready.\n${readLogs()}`);
}
...
test('agency OS APIs complete the lifecycle and preserve tenant isolation', { timeout: 30000 }, async (t) => {
  ...
  const child = spawn(process.execPath, ['server.js'], {
    ...
    env: {
      ...
      TEST_USER_TENANT: 'LessRepeat QA',
  ...
  async function login(email, password) {
    ...
    assert.match(setCookie || '', /^lessrepeat_session=[a-f0-9]{64};/);
  ...
  assert.equal(admin.tenant.name, 'LessRepeat QA');

  const logoutProbe = await login(ADMIN_EMAIL, ADMIN_PASSWORD);
  const beforeLogout = await request(logoutProbe.cookie, '/api/me');
  assert.equal(beforeLogout.response.status, 200);
  assert.equal(beforeLogout.response.headers.get('cache-control'), 'no-store');
  const loggedOut = await request(logoutProbe.cookie, '/api/auth/logout', { method: 'POST', body: {} });
  assert.equal(loggedOut.response.status, 200);
  assert.match(loggedOut.response.headers.get('set-cookie') || '', /lessrepeat_session=;/);
  const afterLogout = await request(logoutProbe.cookie, '/api/me');
  ...
  const adminInvoice = await request(admin.cookie, '/api/invoices', {
    ...
    body: {
      ...
      clientName: 'LessRepeat QA',
      clientEmail: ADMIN_EMAIL,
      description: 'LessRepeat operations retainer',
  ...
  const knowledgeSaved = await request(client.cookie, '/api/knowledge', {
    method: 'POST',
    body: { type: 'faq', name: 'Do you accept walk-ins?', content: 'Yes, subject to same-day availability.' },
  });
  assert.equal(knowledgeSaved.response.status, 200, knowledgeSaved.json.error);
  assert.equal(knowledgeSaved.json.item.status, 'indexed');

  const privateWebsite = await request(client.cookie, '/api/knowledge', {
    method: 'POST',
    body: { type: 'website', name: 'Private target', sourceUrl: 'http://127.0.0.1:8000/internal' },
  });
  assert.equal(privateWebsite.response.status, 422);
  assert.equal(privateWebsite.json.code, 'bad_knowledge_url');

  const textDocument = await request(client.cookie, '/api/knowledge', {
    method: 'POST',
    body: { type: 'document', name: 'Opening hours', fileName: 'hours.txt', mimeType: 'text/plain', size: 31, fileData: Buffer.from('Monday to Saturday, 9 AM to 7 PM.').toString('base64') },
  });
  assert.equal(textDocument.response.status, 200, textDocument.json.error);
  assert.equal(textDocument.json.item.status, 'indexed');
  assert.match(textDocument.json.item.contentPreview, /Monday to Saturday/);
  const textDocumentDeleted = await request(client.cookie, '/api/knowledge/delete', { method: 'POST', body: { id: textDocument.json.item.id } });
  assert.equal(textDocumentDeleted.response.status, 200, textDocumentDeleted.json.error);

  const contactSaved = await request(client.cookie, '/api/contacts', {
    method: 'POST',
    body: { name: 'Asha Rao', phone: '+919876543210', email: 'asha@example.test', status: 'qualified', notes: 'Requested a Friday appointment.' },
  });
  assert.equal(contactSaved.response.status, 200, contactSaved.json.error);
  assert.equal(contactSaved.json.contact.status, 'qualified');

  const automationSaved = await request(client.cookie, '/api/automations', {
    method: 'POST',
    body: { name: 'Route qualified dental leads', trigger: 'lead_qualified', action: 'route_lead', destination: 'Front desk queue', status: 'active' },
  });
  assert.equal(automationSaved.response.status, 200, automationSaved.json.error);
  assert.equal(automationSaved.json.rule.action, 'route_lead');

  const adminKnowledge = await request(admin.cookie, '/api/knowledge');
  const adminContacts = await request(admin.cookie, '/api/contacts');
  const adminAutomations = await request(admin.cookie, '/api/automations');
  assert.deepEqual(adminKnowledge.json.items, []);
  assert.deepEqual(adminContacts.json.contacts, []);
  assert.deepEqual(adminAutomations.json.rules, []);

  const clientKnowledge = await request(client.cookie, '/api/knowledge');
  const clientContacts = await request(client.cookie, '/api/contacts');
  const clientAutomations = await request(client.cookie, '/api/automations');
  assert.deepEqual(clientKnowledge.json.items.map((row) => row.id), [knowledgeSaved.json.item.id]);
  assert.deepEqual(clientContacts.json.contacts.map((row) => row.id), [contactSaved.json.contact.id]);
  assert.deepEqual(clientAutomations.json.rules.map((row) => row.id), [automationSaved.json.rule.id]);
  ...
  assert.equal(persisted.knowledgeItems.length, 1);
  assert.equal(persisted.contacts.length, 1);
  assert.equal(persisted.automationRules.length, 1);
  if (process.platform === 'win32') {
    const mode = (await stat(dbFile)).mode & 0o777;
    assert.ok(mode === 0o666 || mode === 0o600, 'Expected Windows file mode to be writable or read-only map');
  } else {
