# LessRepeat , Build Contract (single source of truth for the swarm)
...
**LessRepeat**. The hook: production voice agents at roughly one rupee, powered by
...
- On boot: if `data/db.json` missing, create it and seed a demo tenant + owner
  (email `demo@rapidx.ai`, password `rapidxvoice`, tenant `LessRepeat Demo`) and migrate any
