/**
 * LessRepeat. Provider-agnostic engine.
 *
...
const KOKORO_MODELS = new Set(['kokoro']);
const GEMINI_TTS_MODELS = new Set(['gemini-2.5-flash-preview-tts']);
const GEMINI_TTS_VOICE_PRESETS = Object.freeze([
  Object.freeze({ id: 'Achird', label: 'Sravani', description: 'Friendly, natural Telugu woman' }),
  Object.freeze({ id: 'Sulafat', label: 'Ananya', description: 'Warm, reassuring Telugu woman' }),
  Object.freeze({ id: 'Gacrux', label: 'Madhavi', description: 'Mature, measured Telugu woman' }),
  Object.freeze({ id: 'Iapetus', label: 'Arjun', description: 'Clear, professional Telugu man' }),
  Object.freeze({ id: 'Zubenelgenubi', label: 'Kiran', description: 'Casual, approachable Telugu man' }),
...
const KOKORO_VOICE_PRESETS = Object.freeze([
  ...
  Object.freeze({ id: 'hm_omega(2)+hm_psi(1)', label: 'Rohan', speed: 1.08, description: '25-year-old casual sales voice' }),
  Object.freeze({ id: 'hm_omega(1)+hm_psi(2)', label: 'Mahesh', speed: 0.90, description: '35-year-old, simple and easy English' }),
  Object.freeze({ id: 'hm_omega+hm_psi', label: 'Rajiv', speed: 0.94, description: 'Balanced, conversational Indian male' }),
  Object.freeze({ id: 'hf_alpha', label: 'Asha', speed: 0.96, description: 'Clear, confident Indian woman' }),
  Object.freeze({ id: 'hf_beta', label: 'Meera', speed: 0.92, description: 'Warm, friendly Indian woman' }),
  Object.freeze({ id: 'hf_alpha(1)+hf_beta(2)', label: 'Kavita', speed: 0.84, description: '40-year-old, warm and measured Indian English' }),
  Object.freeze({ id: 'hf_alpha(2)+hf_beta(1)', label: 'Priya', speed: 0.98, description: 'Professional, natural Indian woman' }),
  Object.freeze({ id: 'hf_alpha+hf_beta', label: 'Nandini', speed: 0.90, description: 'Balanced, approachable Indian woman' }),
]);
const KOKORO_VOICES = new Set(KOKORO_VOICE_PRESETS.map((voice) => voice.id));
...
const ttsKokoro = {
  ...
  async synthesize(opts) {
    const text = String(opts.text || '').slice(0, MAX_TEXT);
    if (!text.trim()) throw new ProviderError('text is required', 422, 'no_text');
    const voice = KOKORO_VOICES.has(opts.speaker) ? opts.speaker : 'hf_alpha';
    const preset = KOKORO_VOICE_PRESETS.find((item) => item.id === voice);
    const speed = opts.speed != null && Number.isFinite(Number(opts.speed))
      ? Math.max(0.75, Math.min(1.2, Number(opts.speed)))
      : (preset ? preset.speed : 1);
    let response;
    try {
      response = await fetch(`${KOKORO_BASE_URL}/v1/audio/speech`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'kokoro', input: text, voice, response_format: 'wav', speed, stream: false, lang_code: 'h' }),
        signal: AbortSignal.timeout(30000),
      });
    } catch (error) {
      throw new ProviderError('local Kokoro service is unavailable', 503, 'kokoro_unavailable', error.message);
    }
    if (!response.ok) {
      throw new ProviderError('Kokoro synthesis failed', response.status, 'upstream', (await response.text()).slice(0, 300));
    }
    return { buffer: Buffer.from(await response.arrayBuffer()), credits: '0', chars: text.length };
  },

  async wsConnect() {
    throw new ProviderError('Kokoro uses Dograh streaming directly', 409, 'stream_via_dograh');
  },
};
...
const ttsGemini = {
  ...
  get live() { return hasEnv(this.needs); },
  get model() { return process.env.GEMINI_TTS_MODEL || 'gemini-2.5-flash-preview-tts'; },
  ...
  async synthesize(opts) {
    ...
    const voice = GEMINI_TTS_VOICES.has(opts.speaker) ? opts.speaker : 'Achird';
    const direction = String(opts.description || 'Natural Telugu business voice agent, warm, concise, conversational, and easy for local callers to understand.').trim().slice(0, 400);
    const input = `Read the following transcript aloud in natural Telugu with an Indian accent. Output speech audio only. Voice direction: ${direction}\n\nTranscript: ${text}`;
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      upstream = await httpsPost(GEMINI_HOST, `/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(key)}`, {
        'Content-Type': 'application/json',
    ...
    let data;
    try { data = JSON.parse(upstream.buffer.toString('utf8')); } catch { data = {}; }
    if (upstream.status < 200 || upstream.status >= 300) {
      throw new ProviderError('Gemini Telugu synthesis failed', upstream.status, 'upstream', String(data?.error?.message || upstream.buffer.toString('utf8')).slice(0, 300));
    }
    const audioPart = ((data?.candidates?.[0]?.content?.parts) || []).find((part) => part?.inlineData?.data || part?.inline_data?.data) || {};
    const inline = audioPart.inlineData || audioPart.inline_data || {};
    const encoded = inline.data || '';
    if (!encoded) throw new ProviderError('Gemini returned no Telugu audio', 502, 'empty_audio');
    const pcm = Buffer.from(encoded, 'base64');
    if (!pcm.length) throw new ProviderError('Gemini returned empty Telugu audio', 502, 'empty_audio');
    const rateMatch = String(inline.mimeType || inline.mime_type || '').match(/rate=(\d+)/i);
    const sampleRate = rateMatch ? Number(rateMatch[1]) : 24000;
    const seconds = pcm.length / (sampleRate * 2);
    return { buffer: pcm16MonoToWav(pcm, sampleRate), credits: `${Math.ceil(seconds * 32)} audio tokens`, chars: text.length };
  },

  async wsConnect() {
    throw new ProviderError('Gemini Telugu uses file synthesis in this MVP', 409, 'file_synthesis_only');
  },
};
...
const DEFAULT_SYSTEM = 'You are LessRepeat, a warm, concise business voice assistant. Answer only within the configured business purpose. Refuse unrelated general knowledge. Reply in 1 to 3 short spoken sentences. No markdown, no lists, no emojis. This will be read aloud.';
