...
const BASE_HEADERS = {
  ...
  'Content-Security-Policy-Report-Only': "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self' http: https: ws: wss:; frame-src 'self' https:; object-src 'none'; base-uri 'self'; frame-ancestors 'self'; form-action 'self' https://secure.payu.in https://test.payu.in",
};
...
async function createSession(userId, tenantId) {
  ...
  await mutate((d) => {
    ...
    const activeForUser = d.sessions
      .filter((s) => s.userId === userId && !s.impersonatorUserId)
      .sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
    const remove = new Set(activeForUser.slice(MAX_SESSIONS_PER_USER - 1).map((s) => s.tokenHash || s.token));
    d.sessions = d.sessions.filter((s) => !remove.has(s.tokenHash || s.token));
    d.sessions.push({ tokenHash, userId, tenantId, createdAt: Date.now(), exp });
  });
  return token;
}
